#!/usr/bin/env python3
# ============================================
# evaluate_wavlm_raw.py - Evaluate Raw WavLM (No Fine-tuning)
# Usage: 
#   python evaluate_wavlm_raw.py --config 5percent --split 1
#   python evaluate_wavlm_raw.py --config 5percent --split 2
# ============================================
...
class RawWavLMExtractor:
    ...
    def encode_batch(self, audio):
        ...
        with torch.no_grad():
            ...
            last_hidden = outputs.last_hidden_state  # [batch, time, 1024]
            
            # Mean pooling over time
            embedding = last_hidden.mean(dim=1)  # [batch, 1024]
            
            # L2 normalize
            embedding = F.normalize(embedding, p=2, dim=1)
            
        # Add extra dimension to match SpeechBrain format [batch, 1, dim]
        return embedding.unsqueeze(1)


def evaluate():
    parser = argparse.ArgumentParser(description='Evaluate Raw WavLM (No Fine-tuning)')
    ...
    print(f"Model: WavLM-Large (Frozen, No Fine-tuning)")
    ...
    # Load Raw WavLM model (completely frozen, no fine-tuning)
    print(f"\n🔧 Loading Raw WavLM (No Fine-tuning)...")
    device = "cuda:0" if torch.cuda.is_available() else "cpu"
    print(f"   Device: {device}")
    
    extractor = RawWavLMExtractor(device=device)
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Extracting embeddings"):
            ...
            batch_emb = batch_emb.squeeze(1)  # [batch, 1024]
    ...
    output_file = os.path.join(args.output_dir, f"wavlm_raw_split{args.split}_results.txt")
    with open(output_file, 'w') as f:
        f.write(f"Raw WavLM (No Fine-tuning) Results\n")
        f.write(f"==================================\n\n")
    ...
    if per_lang_results:
        ...
        axes[1, 1].set_title('Per-Language Equal Error Rate (Raw WavLM)')
    ...
    output_plot = os.path.join(args.output_dir, f"wavlm_raw_split{args.split}_results.png")
...
if __name__ == "__main__":
    ...
    # Print final summary
    print("\n" + "="*70)
    print("📊 FINAL SUMMARY - RAW WAVLM (NO FINE-TUNING)")
    print("="*70)
